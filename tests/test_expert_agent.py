...
@pytest.mark.parametrize(
    "agent_class, expected_prompt_filename, agent_role",
    [
...
class TestExpertAgents:
    ...
    def test_run_success(
        ...
        # 3. 검증 (Assert)
        expected_path = (
            DUMMY_CONFIG["paths"]["prompt_template_dir"] + expected_prompt_filename
        )
    ...
    @patch("maestro.agents.expert_agents.read_text_file")
    @patch("maestro.agents.expert_agents.call_llm")
    def test_run_success_with_json_wrapper(
        self,
        mock_call_llm,
        mock_read_file,
        agent_class,
    ...
    @patch("maestro.agents.expert_agents.call_llm")
    @patch("maestro.agents.expert_agents.read_text_file")
    def test_run_prompt_file_not_found(
        self,
        mock_read_file,
        mock_call_llm,
        agent_class,
    ...
    @patch("maestro.agents.expert_agents.read_text_file")
    @patch("maestro.agents.expert_agents.call_llm")
    def test_run_llm_returns_invalid_json(
        self,
        mock_call_llm,
        mock_read_file,
        agent_class,
    ...
    @patch("maestro.agents.expert_agents.read_text_file")
    @patch("maestro.agents.expert_agents.call_llm")
    def test_run_llm_returns_invalid_schema(
        self,
        mock_call_llm,
        mock_read_file,
        agent_class,
        ...
        invalid_schema_response = [
            {"agent_role": agent_role, "title": "A Test Suggestion"}
        ]
...
def test_security_expert_handles_critical_severity(mocker):
    ...
    # 1. 준비 (Arrange)
    mocker.patch(
        "maestro.agents.expert_agents.read_text_file",
        return_value="Code: {v_gen}\nTests: {unit_test_suite}",
    )
