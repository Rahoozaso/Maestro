...
class PerformanceExpert(BaseAgent):
    ...
    def run(self, code_to_analyze: str, unit_tests: str) -> List[ExpertReviewReport]:
        ...
        # 1. 프롬프트 로드 및 생성
        prompt_path = (
            self.config["paths"]["prompt_template_dir"] + "performance_prompt.md"
        )
        try:
        ...
        prompt = prompt_template.format(
            v_gen=code_to_analyze, unit_test_suite=unit_tests
        )
        messages = [
            {
                "role": "system",
                "content": "You are a world-class expert in Python code performance optimization.",
            },
        ...
        try:
            ...
            validated_reports = [
                ExpertReviewReport.model_validate(item) for item in parsed_data
            ]
...
class ReadabilityExpert(BaseAgent):
    ...
    def run(self, code_to_analyze: str, unit_tests: str) -> List[ExpertReviewReport]:
        ...
        # 1. 프롬프트 로드 및 생성
        prompt_path = (
            self.config["paths"]["prompt_template_dir"] + "readability_prompt.md"
        )
        try:
            prompt_template = read_text_file(prompt_path)
        except FileNotFoundError:
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a world-class expert in Python code readability optimization.",
            },
            {"role": "user", "content": prompt},
        ]
...
class SecurityExpert(BaseAgent):
    ...
    def run(self, code_to_analyze: str, unit_tests: str) -> List[ExpertReviewReport]:
        ...
        messages = [
            {
                "role": "system",
                "content": "You are a world-class expert in Python code security optimization.",
            },
            {"role": "user", "content": prompt},
        ]
