...
def analyze_security(code_string: str) -> SecurityReport:
    ...
    try:
        ...
        command = ["bandit", "-r", temp_filepath, "-f", "json"]
        ...
        print(
            f"보안 분석 완료: {len(issues)}개의 이슈 발견, 최고 심각도: {highest_severity_str or 'None'}"
        )

        return SecurityReport(
            success=True, highest_severity=highest_severity_str, issues=issues
        )
