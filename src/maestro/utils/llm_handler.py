...
def set_llm_provider(config: Dict[str, Any]):
    ...
    if not provider:
        raise ValueError("LLM 설정(config.yml)에 'provider'가 지정되지 않았습니다 (e.g., openai, anthropic).")
...
def call_llm(messages: List[Dict[str, str]], llm_config: Dict[str, Any]) -> str:
    ...
    if _client is None:
        ...
        print("LLM 공급자가 설정되지 않아, llm_config를 사용하여 초기 설정을 시도합니다.")
    ...
    try:
        if _llm_provider == "openai":
            ...
            return '{"status": "mock success", "final_code": "pass", "log": ["mock log"]}'
