...
from src.core.data_models import DeveloperAgentOutput

# 테스트에 사용할 가짜 설정(config)과 입력 데이터
DUMMY_CONFIG = {
    'paths': {'prompt_template_dir': 'src/prompts/'},
    'llm': {'provider': 'dummy'}
}
...
@patch('src.agents.developer_agent.call_llm')
def test_developer_agent_run_success(mock_call_llm):
    """
...
@patch('src.agents.developer_agent.call_llm')
def test_developer_agent_run_invalid_json(mock_call_llm):
    """
...
@patch('src.agents.developer_agent.call_llm')
def test_developer_agent_run_validation_error(mock_call_llm):
    """
